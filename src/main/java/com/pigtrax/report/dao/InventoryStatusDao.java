...
public class InventoryStatusDao {
	...
	public List<InventoryStatusBean> getInventoryList(final Integer premiseId)
	{
		...
		String qry="SELECT T.\"barnId\", T.\"Phase Type\", T.\"Group Id\", T.\"Animal Type\", T.\"Head\", T.\"DOF\" "
				...
				+ "  CASE WHEN PS.\"id_PigletStatusEventType\" = 2 THEN 'Wean' ELSE CASE WHEN PS.\"id_PigletStatusEventType\" = 3 THEN 'Wean' ELSE CASE WHEN PS.\"id_PigletStatusEventType\" = 4 THEN 'Wean' ELSE 'Wean' END eND END "
				+" ELSE CASE WHEN PEM.\"id_FarrowEvent\" > 0 THEN 'Farrow' " 
				+" ELSE  CASE WHEN PEM.\"id_PregnancyEvent\" > 0 THEN "
				+"    CASE WHEN PE.\"id_PregnancyEventType\" = 2 OR PE.\"id_PregnancyEventType\" = 3  OR (PE.\"id_PregnancyEventType\" = 1 AND PE.\"id_PregnancyExamResultType\" = 2) THEN 'Open' ELSE 'BreedingEvent' END  "
				...
				+" WHERE " 
				+" PEM.\"id\" in ( "			
				+" SELECT distinct on (PEM1.\"id_PigInfo\")  PEM1.\"id\" " 
				+" FROM " 
