...
public class FeedReportService {
	...
	public List<String> getFeedList(String premise, int premiseId, Locale locale) {
		...
		if (feedReportBeanList != null && feedReportBeanList.size() > 0) {
			...
			returnRows.add(messageSource.getMessage("label.piginfo.groupEventForm.groupId", null, "", locale)+","+messageSource.getMessage("label.premise.barn", null, "", locale)+","+","+messageSource.getMessage("label.piginfo.feedEventForm.batchId", null, "", locale)+","
