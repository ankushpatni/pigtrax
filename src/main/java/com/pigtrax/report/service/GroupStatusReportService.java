...
public class GroupStatusReportService {
	...
	public List<String> getGroupStatusResult(String premise, Integer premiseId, Date startDate, Date endDate, String groupIdStr, Locale locale, String reportType) { 
		...
		if(groupId != null)
		{
			...
			GroupEvent groupEvent = groupDao.getGroupEventByGeneratedGroupId(groupId, 0);
			
			try {
				List groupEvents = groupEventService.getGroupEventAndDetailByGroupId(groupEvent.getGroupId(), groupEvent.getCompanyId(), groupEvent.getPremiseId(), null);
				if(groupEvents != null)
					groupEvent = (GroupEvent)groupEvents.get(0);
				
				room = roomDao.findByRoomByAutoGeneratedId(Integer.parseInt(groupEvent.getRoomId()));
				barn = barnDao.findByBarnByAutoGeneratedId(room.getBarnId());
			...
			if (rangeList != null && rangeList.size() > 0) {
				...
				for (Map mpRow : rangeList) {
					...
					rowBuffer.append((barn!=null?barn.getBarnId():"")+ seprater);
					rowBuffer.append((room != null ? room.getRoomId():"")+ seprater);
