				...
				if(premise != null)
				{
					...
					{ 
						...
						else
						{
							...
							{
								response.sendRedirect("prodEventLog?nodata=true");
							}
							else
							{
		...
		try {
			...
			try {
				...
				else
				{
					...
					if(fromOverView == null)
					{
						response.sendRedirect("targetReport?nodata=true");
					}
					else
					{
						response.sendRedirect("overViewReport?nodata=true");
				...
				if(premise != null)
				{
					...
					{ 
						...
						else
						{
							...
							if(fromOverView == null)
							{
								response.sendRedirect("rationReport?nodata=true");
							}
							else
							{
								response.sendRedirect("overViewReport?nodata=true");
...
//				String startDate = request.getParameter("startDate");
				...
				java.util.Date startDatedt = endDatedt.toDate();
//				java.util.Date startDatedt  = DateUtil.convertToFormat(endDatedt.toString(), "dd/MM/yyyy");
				...
				try {
					...
					if(premise != null)
					{
						...
						if(premiseId > 0)
						{ 
							rows = gestationReportService.getGestationResult(premise.getPermiseId(), premiseId, startDatedt, DateUtil.convertToFormat(endDate, "dd/MM/yyyy"), localeResolver.resolveLocale(request), reportOption); 
							Iterator<String> iter = rows.iterator();
							if(rows != null && rows.size() > 0)
							{
								while (iter.hasNext()) {
							...
							else
							{
								...
								if(fromOverView == null)
								{
									response.sendRedirect("gestationReport?nodata=true");
								}
								else
								{
									response.sendRedirect("overViewReport?nodata=true");
				...
				try {
					...
					if(premiseId > 0)
					{
						...
						else
						{
							...
							if(fromOverView == null)
							{
								response.sendRedirect("reportGenerationActionList?nodata=true");
							}
							else
							{
								response.sendRedirect("overViewReport?nodata=true");
				...
				try {
					...
					if(premise != null)
					{
						...
						{ 
							...
							{
								request.getSession(true).setAttribute("REPORT_NO_DATA", true);
								response.sendRedirect("reportLactationLength?nodata=true");
							}
						}
						else
						{
				...
				try {
					...
					try {
						...
						if(selectedGroup != null)
							groupId = Integer.parseInt(selectedGroup);
						...
						if(premise != null)
						{
							if(premiseId > 0)
							{ 
								rows = feedReportService.getFeedList(selectedPremise,premiseId, localeResolver.resolveLocale(request), groupId); 
								if(rows == null || rows.size() > 0)
								{
									response.setContentType("text/csv");
		...
		public void generateOverViewReport(HttpServletRequest request, HttpServletResponse response) {
			try {
				...
				try {
					    ...
					    else if(reportType.equalsIgnoreCase("PerformanceMonitor"))
					    {
					    	 rows = getFerrowReports(startDate, endDate, companyId, premiseId); 
						 }
