		...
		try {
			...
			try {
				...
				if(premise != null)
				{
					...
					if(premiseId > 0)
					{ 
						rows = rationReportService.getRationReportList(premise.getPermiseId(), premiseId, DateUtil.convertToFormat(startDate, "dd/MM/yyyy"), DateUtil.convertToFormat(endDate, "dd/MM/yyyy"), groupId); 
						Iterator<String> iter = rows.iterator();
						while (iter.hasNext()) {
							String outputString = (String) iter.next();
							response.getOutputStream().print(outputString);
			...
			try {
				...
				try {
					...
					if(premise != null)
					{
						...
						if(premiseId > 0)
						{ 
							rows = gestationReportService.getGestationResult(premise.getPermiseId(), premiseId, DateUtil.convertToFormat(startDate, "dd/MM/yyyy"), DateUtil.convertToFormat(endDate, "dd/MM/yyyy")); 
							Iterator<String> iter = rows.iterator();
							while (iter.hasNext()) {
								String outputString = (String) iter.next();
								response.getOutputStream().print(outputString);
