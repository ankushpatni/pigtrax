...
private static final Logger logger = Logger.getLogger(CompanyDaoImpl.class);
	...
	public List<Company> getCompanyList() {
		String query = "SELECT \"id\",\"companyId\", \"name\", \"address\", \"city\", \"country\", \"registrationNumber\", \"email\", \"phone\", "
				+ "\"contactName\", \"payment\", \"paymentDate\", \"isActive\",\"otherCity\" from pigtrax.\"Company\" order by \"id\" desc ";
	...
	public Company findByCompanyID(final String companyID) {
		String query = "SELECT \"id\",\"companyId\", \"name\", \"address\", \"city\",\"country\", \"registrationNumber\", \"email\", "
				+ "\"phone\", \"contactName\", \"payment\", \"paymentDate\", \"isActive\",\"otherCity\" from pigtrax.\"Company\"  WHERE \"companyId\"=?";
	...
	public Company findByCompanyByAutoGeneratedId(final int generatedCompanyId)
			throws SQLException {
		String query = "SELECT \"id\",\"companyId\", \"name\", \"address\", \"city\",\"country\", \"registrationNumber\", \"email\", \"phone\", "
				+ "\"contactName\", \"payment\", \"paymentDate\", \"isActive\",\"otherCity\" from pigtrax.\"Company\"  WHERE \"id\"=?";
		List<Company> companyList = jdbcTemplate.query(query, new PreparedStatementSetter(){
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
	...
	public int insertCompanyRecord(final Company company) {
		String query = "INSERT INTO pigtrax.\"Company\"(  \"companyId\", name, address, city, \"registrationNumber\", email, phone,"
					 +" \"contactName\", payment, \"paymentDate\", \"isActive\", \"lastUpdated\",\"userUpdated\",\"country\",\"otherCity\")"+
					 "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?)";
		...
		return this.jdbcTemplate.update(query, new PreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps) throws SQLException {
				...
				ps.setString(15, company.getOtherCity());
			}
		});
	}
	
	public int updateCompanyRecord(final Company company) {
		String query = "update pigtrax.\"Company\" SET name=?, address=?, city =?, \"registrationNumber\" =?, email=?, phone=?, \"contactName\"=?, payment=?, \"paymentDate\"=?, \"lastUpdated\"=?, "
				+ "\"userUpdated\"=?, \"country\"=?,\"otherCity\" = ?  WHERE \"companyId\"=?";
		return this.jdbcTemplate.update(query, new PreparedStatementSetter() {
			@Override
			public void setValues(PreparedStatement ps) throws SQLException {
				...
				ps.setString(13, company.getOtherCity());
				ps.setString(14,company.getCompanyId());
	...
	private static final class CompanyMapper implements RowMapper<Company> {
		public Company mapRow(ResultSet rs, int rowNum) throws SQLException {
			...
			company.setOtherCity(rs.getString("otherCity"));
